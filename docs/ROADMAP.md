...
| 1 | Fondations transverses | 0.2.0 | À faire | `core:model` (AppResult, AppError, identifiants typés, StorageLocation), `core:domain` (DispatcherProvider, conventions des use cases), `core:testing` (premiers fakes), `core:ui` (thème Material 3 complet, BaseFragment, composants d'état, AppNavigator), `app` (CodeIdeApplication Hilt + StrictMode/LeakCanary, SplashScreen, NavHost, navigation Home ↔ Settings) |
| 2 | Journalisation de l'application | 0.3.0 | À faire | Section 5.7 : AppLogger, LogRedactor, sinks Logcat/fichier JSONL, rotation, écriture asynchrone, breadcrumbs, export zip, FileProvider |
| 3 | Gestion des plantages | 0.4.0 | À faire | Section 5.8 : CrashHandler, CrashActivity (processus `:crash`), ApplicationExitInfo, boucle de plantages, menu debug |
| 4 | Couche données | 0.5.0 | À faire | `core:domain` (repositories, FileSystem + use cases), `core:model` (Project, AppSettings), `core:datastore`, `core:database` (Room v1), `core:storage` (SafFileSystem), `core:data`, fakes |
| 5 | Assistant de premier lancement | 0.6.0 | À faire | 5 écrans : bienvenue, dossier de travail SAF (test d'écriture), apparence (aperçu immédiat), profil, terminé |
