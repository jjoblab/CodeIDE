...
| 31 | Explorateur de fichiers v2 — tiroir à fragments | 0.32.0 | — | Reproduction **à l'identique** de la preview HTML validée (`docs/preview/explorateur-v2.html`, spécification `docs/EXPLORATEUR_V2.md`) : tiroir en **fragments** (Explorateur/Recherche/Git/Terminal, entête propre par fragment, plus d'entête commun), poignée ⋮ de redimensionnement (bornes 45–98 %, aimants 55/69/85/98 %), arbre treeview (guides fins, chevrons de dépliage, points d'état à 4 états), vraies icônes par type de fichier, bascule **Projet/Privé** exclusive, popover maison ancré au doigt avec chemin contextuel, mutations **par nœud** (créer/renommer/supprimer/copier/couper/coller/déplacer — ADR 0027/0030 respectées) |
